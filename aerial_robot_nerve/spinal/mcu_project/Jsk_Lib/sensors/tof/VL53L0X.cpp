...
// provided by ST (ST  tof_sensor_.init(&hi2c2, &nh_);
//SW-IMG005), and some of the explanatory comments are quoted
...
#define startTimeout() (timeout_start_ms_ = HAL_GetTick())

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout_ > 0 && ((uint32_t)HAL_GetTick() - timeout_start_ms_) > io_timeout_)
...
// based on VLset53L0X_calc_macro_period_ps()
...
VL53L0X::VL53L0X(void)
  : range_pub_("/vl53l0x/range", &range_msg_)
  , offset_sub_("/vl53l0x/set_offset", &VL53L0X::setOffsetCallback, this )
...
// mode.
bool VL53L0X::init(I2C_HandleTypeDef* hi2c, ros::NodeHandle* nh, bool single_shot_mode, bool io_2v8)
{
  i2c_handle_ = hi2c;
  nh_ = nh;
  nh_->advertise(range_pub_);
  nh_->subscribe< ros::Subscriber<std_msgs::Float32, VL53L0X> >(offset_sub_);
  FlashMemory::addValue(&offset_, sizeof(float));

  single_shot_mode_ = single_shot_mode;
  ...
  if(!single_shot_mode_) startContinuous(); // ms
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  ...
  */
  ...
  val[0] = reg;
  val[1] = value;
  HAL_I2C_Master_Transmit(i2c_handle_, address_, val, 2, 100);
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  ...
  */
  ...
  val[1] = (value >> 8) & 0xFF;
  val[2] = value & 0xFF;
  HAL_I2C_Master_Transmit(i2c_handle_, address_, val, 3, 100);
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  ...
  */
  ...
  val[2] = (value >> 8) & 0xFF;
  val[4] = value & 0xFF;
  HAL_I2C_Master_Transmit(i2c_handle_, address_, val, 5, 100);
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  ...
  */
  ...
  //uint8_t reg_p[1];
  //reg_p[0] = reg;
  HAL_I2C_Master_Transmit(i2c_handle_, address_, &reg, 1, 100);
  HAL_I2C_Master_Receive(i2c_handle_, address_, val, 1, 100);
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  */
  ...
  //reg_p[0] = reg;
  HAL_I2C_Master_Transmit(i2c_handle_, address_, &reg, 1, 100);
  HAL_I2C_Master_Receive(i2c_handle_, address_, val, 2, 100);
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  */
  ...
  HAL_I2C_Master_Transmit(i2c_handle_, address_, &reg, 1, 100);
  HAL_I2C_Master_Receive(i2c_handle_, address_, val, 4, 100);
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const * src, uint8_t count)
{
  ...
  */
  ...
  uint8_t *p;
  p = (uint8_t *)malloc(sizeof(uint8_t) * (1 + count));
  p[0] = reg;
  for(int i = 0; i < count; i++) p[1 + i] = src[i];

  HAL_I2C_Master_Transmit(i2c_handle_, address_, p, 1 + count, 100);
  free(p);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  ...
  */
  ...
  HAL_I2C_Master_Transmit(i2c_handle_, address_, &reg, 1, 100);
  HAL_I2C_Master_Receive(i2c_handle_, address_, dst, count, 100);
}
...
#else
  if((readReg(RESULT_INTERRUPT_STATUS) & 0x07) == 0) return 65535;
#endif
...
uint16_t VL53L0X::readRangeSingleMillimeters(void)
{
  ...
  if(readReg(SYSRANGE_START) & 0x01) return 65535;
  else
  {
	 uint16_t range = readRangeContinuousMillimeters();
...
void VL53L0X::setOffsetCallback(const std_msgs::Float32& cmd_msg)
{
	offset_ = cmd_msg.data;
	FlashMemory::erase();
	FlashMemory::write();
    nh_->loginfo("set offset for the ToF sensor VL53L0X");
}
