...
class Ceilingavoid():
  ...
  def __init__(self):
    ...
    self.max_height = rospy.get_param("~max_height", 6)#2.0
    self.min_height = rospy.get_param("~min_height", 0)#1.0
    self.hovering_height = rospy.get_param("~hovering_height",0.62)
    ...
    self.rate = rospy.get_param("~main_rate", 20.0) #20Hz
  ...
  def main(self):
    ...
    while not rospy.is_shutdown():
        ...
        if self.flag == 1:
          rospy.loginfo("reach max height, descend")
          ...
          rospy.loginfo("ceiling detected, descending")
          # if self.now_z is None:
          #   continue

          self.linear_move_sub.unregister()
          target_alt = self.min_height # self.now_z - self.avoid_distance
          msg.pos_z_nav_mode = msg.POS_MODE
