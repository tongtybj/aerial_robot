...
void Base::freeLegAction()
{
  ...
  if (raise_leg_flag_) {
    ...
    for (int j = 0; j < leg_num; j++) {
      ...
      int i = j * 4 + 1;
      target_joint_state_.position.at(i) = prev_target_joint_angles_.at(i) - raise_angle_;

      if (raise_converge_) continue;

      // raise separate motion
      i = j * 4; // inside yaw joint of the free leg
      target_joint_state_.position.at(i) = prev_target_joint_angles_.at(i);
      i = j * 4 + 3; // outside pitch joint of the free leg
      target_joint_state_.position.at(i) = prev_target_joint_angles_.at(i);
    }


    // check raise convergence
    if (!raise_converge_){
      ...
      if (all_raise_converge && !raise_converge_) {
        ...
        updateJoinAngleFrominverseKinematics(true); // update the target joint angle for inside yaw and outside pitch
      }
    }
  }
...
void Base::raiseLeg(int leg_id)
{
  ...
  // set the inside ptich joint of free leg to be identical to the current angle
  const auto current_joint_angles = getCurrentJointAngles();
  const int leg_num = spidar_robot_model_->getRotorNum() / 2;
  for (int j = 0; j < leg_num; j++) {
    ...
    int i = j * 4 + 1;
    prev_target_joint_angles_.at(i) = current_joint_angles.at(i);
  }
}
