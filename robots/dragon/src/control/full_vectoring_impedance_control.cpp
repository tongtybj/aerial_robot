#include <dragon/control/full_vectoring_impedance_control.h>
...
namespace
{
  ...
  void wrenchAllocationEqCons(unsigned m, double *result, unsigned n, const double* x, double* grad, void* ptr)
  {
    ...
    */
    ...
    DragonFullVectoringImpedanceController *controller = reinterpret_cast<DragonFullVectoringImpedanceController*>(ptr);
...
DragonFullVectoringImpedanceController::DragonFullVectoringImpedanceController():
  PoseLinearController()
{
}

void DragonFullVectoringImpedanceController::initialize(ros::NodeHandle nh, ros::NodeHandle nhp,
...
void DragonFullVectoringImpedanceController::reset()
{
...
void DragonFullVectoringImpedanceController::rotorInterfereCompensation()
{
...
void DragonFullVectoringImpedanceController::controlCore()
{
...
//  std::cout<<"be_pos_cog"<<be_pos_cog<<std::endl;
  ...
  for(int i = 0; i < 3; i++)
    {
      if(fabs(target_vel_[i]) > gimbal_roll_target_lin_vel_thresh_)
        target_lin_acc_w_low_freq[i] = lin_acc_cmd(i);
    }

  tf::Vector3 target_lin_acc_w_high_freq((-Kdt(0) * delta_v(0)),
                                         (-Kdt(1) * delta_v(1)),
                                         (-Kdt(2) * delta_v(2)));
  tf::Vector3 residual = target_lin_acc_w_low_freq + target_lin_acc_w_high_freq - target_lin_acc_w;
  target_lin_acc_w_high_freq -= residual;
  ...
  for(int i = 0; i < 3; i++)
    {
      if(fabs(target_omega_[i]) > gimbal_roll_target_ang_vel_thresh_)
        target_ang_acc_low_freq[i] = ang_acc_cmd[i];
    }

  tf::Vector3 target_ang_acc_high_freq((-Kd * delta_v.segment(3, 3))[0],
                                       (-Kd * delta_v.segment(3, 3))[1],
                                       (-Kd * delta_v.segment(3, 3))[2]);
  residual = target_ang_acc_low_freq + target_ang_acc_high_freq - target_ang_acc;
  target_ang_acc_high_freq -= residual;
  Eigen::VectorXd target_acc_low_freq = Eigen::VectorXd::Zero(6);
  Eigen::VectorXd target_acc_high_freq = Eigen::VectorXd::Zero(6);
...
void DragonFullVectoringImpedanceController::externalWrenchEstimate()
{
...
bool DragonFullVectoringImpedanceController::strictNonlinearAllocation(const Eigen::VectorXd target_acc, const std::map<std::string, Dragon::ExternalWrench>& external_wrench_map, const std::vector<Eigen::Vector3d>& extra_vectoring_forces, KDL::JntArray& gimbal_processed_joint, const std::vector<Eigen::Matrix3d>& links_rotation_from_cog)
{
...
bool DragonFullVectoringImpedanceController::gradientDescentAllocation(const int iterative_cnt, const Eigen::VectorXd target_acc, const std::map<std::string, Dragon::ExternalWrench>& external_wrench_map, const std::vector<Eigen::Vector3d>& extra_vectoring_forces, KDL::JntArray& gimbal_processed_joint, const std::vector<Eigen::Matrix3d>& links_rotation_from_cog, std::vector<double>& thrust_forces, std::vector<double>& gimbal_angles)
{
  if (external_wrench_map.size() > 0) ROS_ERROR("Currently strictNonlinearAllocation does not support the external wrench compensation");
...
bool DragonFullVectoringImpedanceController::srInverseAllocation(const Eigen::MatrixXd& q_wrench, const Eigen::VectorXd& nominal_x, const Eigen::VectorXd& target_wrench, const Eigen::VectorXd& nominal_thrust_force, std::vector<double>& thrust_force, std::vector<double>& gimbal_angles)
{
...
Eigen::VectorXd DragonFullVectoringImpedanceController::calcExternalWrenchSum(const std::map<std::string, Dragon::ExternalWrench>& external_wrench_map)
{
...
/* external wrench */
void DragonFullVectoringImpedanceController::addExternalWrenchCallback(const aerial_robot_msgs::ApplyWrench::ConstPtr& msg)
{
  dragon_robot_model_->addExternalStaticWrench(msg->name, msg->reference_frame, msg->reference_point, msg->wrench);
}

void DragonFullVectoringImpedanceController::clearExternalWrenchCallback(const std_msgs::String::ConstPtr& msg)
{
  dragon_robot_model_->removeExternalStaticWrench(msg->data);
}

/* extra vectoring force  */
void DragonFullVectoringImpedanceController::extraVectoringForceCallback(const aerial_robot_msgs::ForceListConstPtr& msg)
{
...
void DragonFullVectoringImpedanceController::eePosCallback(const geometry_msgs::PointStamped::ConstPtr& msg)
{
...
void DragonFullVectoringImpedanceController::rosParamInit()
{
  ros::NodeHandle control_nh(nh_, "controller");
  ...
  getParam<double>(control_nh, "max", max_, 10.0);
  getParam<double>(control_nh, "cax", cax_, 20.0);
  getParam<double>(control_nh, "kax", kax_, 10.0);
...
PLUGINLIB_EXPORT_CLASS(aerial_robot_control::DragonFullVectoringImpedanceController, aerial_robot_control::ControlBase);
